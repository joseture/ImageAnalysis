...
def process_gray_image(img):
    ...
    """
    # Do something here:
    prew_img = prewitt(img)
    thres_otsu = threshold_otsu(prew_img)
    thres_bin = prew_img < thres_otsu
    proc_img = thres_bin.copy()
    
    # With the mean filter 
    # size = 10
    # footprint = np.ones([size, size])
    # med_img = median(prew_img, footprint)
